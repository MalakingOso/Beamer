...
a server; that is the whole point of this task. Everything directly under
the config root is machine-local by design, and `sync_client` touches none
of it: it only reads `config.sync.url`.

(A `sync/attachments/` directory may exist on machines that ran a Beamer
with file attaching; that feature was removed, nothing reads or writes it
anymore, and it can be deleted by hand. It was never carried by this
protocol — the Syncthing design that would have moved it was never
installed.)
...
Everything syncs through `notes.automerge`: notes, tasks, and the vocabulary
string. There is no second channel and no second mechanism — an earlier
design carried attachment bytes via Syncthing, but file attaching was
