...
- [ ] **Self-update zip is binary-only, assets can go stale.** `src/update/mod.rs` downloads and swaps just the `beamer` binary. Manganis content-hashes asset filenames (`assets/styles-*.css`, the icon), so a release that changes either will self-update the binary but leave the old/missing files at `~/.local/lib/Beamer/assets/` (or wherever assets live) — the app comes up unstyled until a fresh `deploy/install-linux.sh` run. Not worth solving for a two-machine setup; see `agent_docs/running_on_bearcave.md`'s "Linux install location" section.
